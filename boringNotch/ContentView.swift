...
struct ContentView: View {
  ...
  timeline.date.timeIntervalSince(musicManager.timestampDate)
                                      return min(max(musicManager.elapsedTime +
  (delta * musicManager.playbackRate), 0), musicManager.songDuration)
                                  }()
                                  let line: String = {
  ...
  musicManager.currentLyrics.trimmingCharacters(in: .whitespacesAndNewlines)
                                      return trimmed.isEmpty ? "No lyrics found"
                      ...
                      Rectangle()
                          .fill(
                              Defaults[.coloredSpectrogram]
                                  ? Color(nsColor:
  musicManager.avgColor).gradient
                                  : Color.gray.gradient
                          )
                          .frame(width: 50, alignment: .center)
                          .matchedGeometryEffect(id: "spectrum", in:
  albumArtNamespace)
                          .mask {
                              AudioSpectrumView(isPlaying:
